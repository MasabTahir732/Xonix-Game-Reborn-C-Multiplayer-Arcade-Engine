...
* **Game State Management:** Handles dynamic switching between the `Main Menu`, `Login_Page`, `Inventory_Page`, and `LeaderBoardPage`.
...
* **Multiplayer & Matchmaking:** Features custom logic for player pairing (`Match_making.h`) and localized multiplayer modes (`Multiplayer_Page.h`).
* **Persistence Engine:** Uses file I/O (`SaveGame.h`, `saved_game.txt`) to maintain player progress, friend lists (`Friends.h`), and persistent high scores.
